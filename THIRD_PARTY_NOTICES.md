...
当前 app 与验证 harness 尚未分发第三方源码或二进制依赖。Apple 系统框架由目标操作系统提供，不在本文件重复列出。

下表是研究记录，不表示其代码已被复制或随 MacEase 分发：

| Project | Snapshot / license observation | MacEase usage |
|---|---|---|
| [`api-enhanced`](https://github.com/NeteaseCloudMusicApiEnhanced/api-enhanced) | implementation snapshot `5b780addbafe`; relevant paths rechecked at `a7e8d485f31c`, MIT, 2026-08-26 | 从本地克隆核对实际使用端点的模块及 `util/option.js`、`util/request.js`、`util/config.json`；用于协议对照和 golden vectors，不嵌入 server、解灰、签到或 IP 功能 |
