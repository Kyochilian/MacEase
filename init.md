...
- 2026-08-13 更新：功能范围已扩展至红心/歌单写操作、听歌排行、每日推荐、榜单、相似歌曲、推荐歌单与心动模式，按批次实现并逐个登记合同字段，见 `docs/roadmap-features.md`。推荐数据在启动时一次性预取，之后仅用户手动刷新；仍不自动重试、不后台轮询。私人 FM 与 trash 维持不做。
