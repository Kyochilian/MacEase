...
extension PlaybackController {
  ...
  package func outputStartedPlaying() {
    ...
    if lifecycle == nil {
      ...
      let instance = PlaybackLifecycleInstance(
        ...
        track: track,
        context: context,
        downloaded: activeOfflineDownloadID != nil
      )
  ...
  package func finishLifecycle(end: ScrobbleEnd = .interrupted) {
    pauseLifecycleClock()
    guard let lifecycle else { return }
    self.lifecycle = nil
    let played =
      lifecycle.accumulatedSeconds.isFinite
    ...
    let wholeSeconds =
      bounded >= TimeInterval(Int.max)
      ? Int.max : Int(bounded.rounded(.down))
    onLifecycleEvent?(
      .finished(lifecycle.instance, playedSeconds: wholeSeconds, end: end)
    )
  }
